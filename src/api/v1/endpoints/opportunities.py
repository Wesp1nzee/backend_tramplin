...
from src.api.v1.deps import RoleChecker, get_db, reusable_oauth2
...
@router.get(
    ...
    description=(
        ...
        "**Избранные компании:**\n"
        "- Для авторизованных пользователей маркеры компаний из избранного помечаются флагом `is_favorite_company`\n"
        "- Фронтенд может отображать такие маркеры отдельным цветом/иконкой\n\n"
...
# ════════════════════════════════════════════════════════════
#  Moderation Endpoints (Curator only)
# ════════════════════════════════════════════════════════════


@router.get(
    "/moderation/pending",
    response_model=ModerationOpportunityListResponse,
    status_code=status.HTTP_200_OK,
    summary="Вакансии на модерации",
    description=(
        "Получение списка вакансий, ожидающих проверки куратором.\n\n"
        "**Требуемая роль:** CURATOR\n\n"
        "Возвращает вакансии со статусом PLANNED (отправлены на модерацию):\n"
        ...
        "Результат отсортирован по дате создания (старые первыми)."
    ),
)
async def get_pending_moderation(
    ...
    """
    return await opportunity_service.get_pending_moderation_list(
        limit=limit,
        offset=offset,
    )
...
@router.get(
    "/moderation/{opportunity_id}",
    response_model=ModerationOpportunityDetail,
    status_code=status.HTTP_200_OK,
    summary="Детали вакансии для модерации",
    description=(
        "Получение полной информации о вакансии для проверки куратором.\n\n"
        "**Требуемая роль:** CURATOR\n\n"
        "Возвращает все поля вакансии включая:\n"
        "- Описание, требования, обязанности\n"
        "- Навыки и теги\n"
...
async def get_moderation_detail(
    ...
    return await opportunity_service.get_moderation_detail(opportunity_id=opp_uuid)


@router.post(
    "/moderation/{opportunity_id}/review",
    response_model=ModerationReviewResponse,
    status_code=status.HTTP_200_OK,
    summary="Проверка вакансии куратором",
    description=(
        "Проверка вакансии куратором (одобрение или отклонение).\n\n"
        "**Требуемая роль:** CURATOR\n\n"
        "**Параметры:**\n"
        "- `approve` — True для одобрения, False для отклонения\n"
        "- `comment` — комментарий модератора (виден работодателю)\n\n"
        "**Логика:**\n"
        "- Если `approve=true`: статус → ACTIVE, is_moderated → True, published_at → now\n"
        "- Если `approve=false`: статус → DRAFT, is_moderated → False\n"
        "- Работодатель получает уведомление о результате (NotificationType.SYSTEM)\n\n"
        "**Проверки:**\n"
        "- Вакансия должна быть в статусе PLANNED\n"
        "- Вакансия не должна быть уже промодерирована"
    ),
)
async def review_opportunity(
    ...
    return await opportunity_service.review_opportunity(
        opportunity_id=opp_uuid,
